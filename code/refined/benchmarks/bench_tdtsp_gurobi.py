"""
TD-TSP benchmark for the cluster-based Gurobi backend (canonical TD-TSP
benchmark; the autonomous single-MTZ reference lives in
bench_tdtsp_gurobi_autonomous.py).

Runs ClusterTDTSPGurobiSolver across all four time slots
...
For each (size, slot) pair the benchmark prints:
  ...
  - cluster decomposition (n_clusters, raw vs 2-opt-refined distance)
...
results/results_tdtsp_gurobi.json keyed by size.
...
from solvers import ClusterTDTSPGurobiSolver  # noqa: E402

INSTANCES_DIR = REPO_ROOT / "data" / "instances"
RESULTS_PATH = REPO_ROOT / "results" / "results_tdtsp_gurobi.json"
...
def _print_header(instance: dict) -> None:
    ...
    print("\n" + "=" * 100)
    print(f"TD-TSP Cluster-Gurobi  -  n={instance['n']}  city={city}  tz={tz}  "
...
def _print_schedule(result: dict, slot: dict) -> None:
    print(f"\n--- {slot['label']}  (start {result['tour_start_local']} local, "
          ...
          f"clusters={result.get('n_clusters', 1)}) ---")
...
def _print_cross_size_summary(by_size: dict) -> None:
    print(f"\n{'=' * 100}")
    print("Cross-size summary (real-world NYC TD-TSP, cluster-Gurobi, 4 time slots each)")
    print("=" * 100)
...
def _run_one_instance(instance: dict) -> list:
    ...
    solver = ClusterTDTSPGurobiSolver(
