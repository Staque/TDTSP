"""
TD-TSP benchmark for the cluster-based D-Wave backend.

Runs ClusterTDTSPDWaveSolver across all four time slots
...
For each (size, slot) pair the benchmark prints:
  ...
  - solver wall-clock instrumentation (Hybrid CQM is queue + solve bound)
...
results/results_tdtsp_dwave.json keyed by size.

D-Wave Hybrid CQM time limit is 10 seconds per cluster (matches the static
benchmark configuration). For n=100 the solver makes roughly 7 cluster calls
per slot, so a complete run uses about 4 minutes of hybrid solver time.
...
from solvers import ClusterTDTSPDWaveSolver  # noqa: E402

INSTANCES_DIR = REPO_ROOT / "data" / "instances"
RESULTS_PATH = REPO_ROOT / "results" / "results_tdtsp_dwave.json"
...
def _print_header(instance: dict) -> None:
    ...
    print("\n" + "=" * 100)
    print(f"TD-TSP Cluster-D-Wave  -  n={instance['n']}  city={city}  tz={tz}  "
...
def _print_per_slot_summary(rows: list, metric: str) -> None:
    ...
    print("-" * 100)
    success = [r for r in rows if r["status"] in ("optimal", "heuristic")]
...
def _print_cross_size_summary(by_size: dict) -> None:
    print(f"\n{'=' * 100}")
    print("Cross-size summary (real-world NYC TD-TSP, cluster-D-Wave, 4 time slots each)")
    print("=" * 100)
...
def _run_one_instance(instance: dict) -> list:
    ...
    solver = ClusterTDTSPDWaveSolver(
...
def main():
    ...
    with open(RESULTS_PATH, "w", encoding="utf-8") as f:
        json.dump(
            {
                "solver": "Cluster-D-Wave (K-means + Hybrid CQM + 2-opt)",
                "max_cluster_size": MAX_CLUSTER_SIZE,
                "time_limit_per_cluster_seconds": TIME_LIMIT_PER_CLUSTER,
                "by_size": by_size,
            },
            f,
            indent=2,
