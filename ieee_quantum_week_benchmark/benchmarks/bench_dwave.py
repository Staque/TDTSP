"""D-Wave benchmark for paper - Using Cluster Approach for ALL sizes"""
...
load_dotenv()
from solvers.tsp_cluster_dwave_solver import ClusterDWaveTSPSolver
...
for n in SIZES:
    ...
    try:
        # Use Cluster-D-Wave for ALL sizes
        max_cluster = min(15, n)  # D-Wave handles larger clusters well
        solver = ClusterDWaveTSPSolver(max_cluster_size=max_cluster, use_local_search=True, time_limit_per_cluster=10, verbose=False)
        start = time.time()
...
with open(os.path.join(os.path.dirname(__file__), "results_dwave.json"), "w") as f:
    json.dump(results, f, indent=2)
print("\n[OK] Saved to results_dwave.json")
