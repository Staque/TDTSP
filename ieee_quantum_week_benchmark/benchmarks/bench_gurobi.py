"""Cluster-Gurobi benchmark for paper - Using Cluster Approach for ALL sizes"""
...
load_dotenv()
from solvers.tsp_cluster_gurobi_solver import ClusterGurobiTSPSolver
...
for n in SIZES:
    ...
    try:
        # Use Cluster-Gurobi for ALL sizes (consistent with quantum solvers)
        # Max cluster size = 15 for Gurobi (handles larger clusters efficiently)
        max_cluster = min(15, n)
        solver = ClusterGurobiTSPSolver(max_cluster_size=max_cluster, use_local_search=True, verbose=False)
        start = time.time()
...
with open(os.path.join(os.path.dirname(__file__), "results_gurobi.json"), "w") as f:
    json.dump(results, f, indent=2)
print("\n[OK] Saved to results_gurobi.json")
